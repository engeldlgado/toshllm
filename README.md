...
> **RAM warning:** Dynamic MoE reduces **VRAM** by keeping the expert pool addressable in host **RAM**, and Metal wires those pages so the system cannot page or compress them. On machines under 64 GB the expert pool must fit in a third of physical RAM, which on a 32 GB machine is about 10.7 GiB and admits an 11.44 GiB GGUF but not a 19.45 GiB one. Measured on 32 GB, a 9.6 GiB pool runs flat while 12.7 and 16.9 GiB starve the compositor. On 64 GB and larger hosts the cap instead leaves the larger of 16 GiB or 12.5% of RAM unwired, so a 192 GB Mac Pro can pin 68–120 GB MoE banks. If the headroom is unavailable, Automatic mode rejects Dynamic MoE and returns to normal `ncmoe` instead of relying on swap. Dynamic MoE is still single-GPU: D24/split and router stay on `ncmoe`.
