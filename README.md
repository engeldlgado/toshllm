...
- **Vision on the experimental engine:** image input works reliably on the **bundled engine** (Flash Attention left on auto, so the vision encoder self-detects what the GPU supports). The **TurboQuant (experimental)** engine forces Flash Attention on for the LLM, which leaks into the vision encoder and **crashes Gemma 4 (MoE) image encoding** on AMD. Use the bundled engine for Gemma 4 vision; other vision models (e.g. Gemma 3) work on both.
