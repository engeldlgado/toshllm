...
/// Side by side when there is room, stacked otherwise. ViewThatFits builds every
/// candidate to measure it, so two cards get built twice on each layout pass and
/// their hover and focus state ends up split across the copy that is not shown.
